# Tracked Threads — last updated 2026-06-30 13:00 Europe/Rome
...
- **30 Jun 13:00 re-check:** Fable 5 still suspended (Day 18), unchanged from the 07:00 re-check. anthropic.com/news re-fetched — newest still "Introducing Claude Tag" (23 Jun); the 12 Jun suspension statement still the headline story, no dated restoration. Search ("Anthropic Fable 5 access restored June 30 2026") still surfaces only the already-shipped 26 Jun Mythos re-release (TheStreet/govconwire), Manifold/Polymarket odds, an explainx.ai status blog ("Fable 5 is not back"), and an Anthropic X post quoting the 26 Jun Mythos text (social → hard-banned) — the search summary itself: "Fable 5 remains suspended for all general users." Legion docket still pre-ruling. Watch-for unmet → no ADVANCE. (07:00 SKIPPED on the same status; all three 28 Jun and 29 Jun slots likewise.)
