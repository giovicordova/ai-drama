# Tracked Threads — last updated 2026-06-29 20:00 Europe/Rome
...
- **29 Jun 20:00 re-check:** Fable 5 still suspended (Day 17, inside the US-lab launch window). anthropic.com/news/fable-mythos-access re-fetched — still only the 12 Jun text ("working to restore access as soon as possible"), no dated addendum; newsroom newest still "Introducing Claude Tag," 23 Jun. Search ("Anthropic Fable 5 access restored June 29 2026") surfaces only an X post quoting the 26 Jun Mythos-redeployment text (social → hard-banned), the Axios "return soon" scoop (27 Jun, anticipatory/secondary), Polymarket odds (prediction market), and aggregator headlines the anthropic.com pages contradict — the search summary itself states Fable 5 "remains offline" / "remains suspended for all general users" as of 29 Jun. Legion docket still pre-ruling (no PI grant/deny, no government response since the 23–24 Jun filing). Watch-for unmet → no ADVANCE. (07:00, 13:00, and all three 28 Jun slots SKIPPED on the same status.)
