# Tracked Threads — last updated 2026-06-28 20:00 Europe/Rome
...
- **28 Jun 20:00 re-check (US-lab launch window):** Fable 5 still suspended. anthropic.com/news/fable-mythos-access re-fetched — still only the 12 Jun text, no dated addendum; newsroom newest still "Claude Tag," 23 Jun. Search ("Fable 5 access restored June 28") returns only the already-shipped 26 Jun Mythos re-release, the Axios "return soon" scoop (anticipatory/secondary), and an Anthropic X post restating the Mythos re-release (social, hard-banned, not new) — "as of June 28, no full restoration has occurred." Legion docket still pre-ruling (no PI grant/deny, no government response since the 23 Jun filing). Watch-for unmet → no ADVANCE. (Same status held at 07:00 and 13:00 re-checks earlier today; both SKIPPED.)
