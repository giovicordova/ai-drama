...
- **Watch for:** the Qwen3.8-Max open weights actually landing on `huggingface.co/Qwen` (no repo yet as of 4 Aug — would let a downloadable-weights beat ship); an **independent** benchmark from an allowlisted specialist (e.g. Artificial Analysis Intelligence Index) that fixes standing vs GPT-5.6 Sol / Claude Fable 5; the qwen.ai SPA / an allowlisted Tier-2 rendering the benchmark table so the "beats Sol + Fable 5 on 7 evals" claim becomes shippable; or a material pricing/availability change. Don't re-post the bare 2 Aug launch or the 2.4T figure. (Rechecked 4 Aug 20:00 — watch-for still unmet: no `huggingface.co/Qwen` weights repo (HF `author=Qwen` shows only 3/3.5/3.6; open weights still reported for the week of 10 Aug, with a Qwen3.8-27B checkpoint); **no independent allowlisted eval** — `artificialanalysis.ai/models/qwen3-8-max` 404s and the AA changelog has no Qwen3.8-Max entry, so the "Intelligence Index 53" on non-allowlisted aggregators is unverifiable on a primary; qwen.ai SPA still renders only "Qwen" to the fetcher, so the self-reported table (OSWorld-Verified 86.1 / Terminal-Bench 2.1 86.6 / IFBench 82.8 vs GPT-5.6 Sol / Fable 5) stays in non-allowlisted secondary + paywalled Bloomberg only.)
- **Receipts:**
