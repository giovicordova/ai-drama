# Tracked Threads — last updated 2026-06-23 20:00 Europe/Rome
...
- **One-line state:** On 22 Jun OpenAI expanded its Daybreak security platform (launched 11 May) with a cyber-specialised model, GPT-5.5-Cyber, plus Codex Security (an agentic harness that runs models inside repos to find/validate/patch vulnerabilities) and "Patch the Planet" (an open-source-maintainer program with Trail of Bits). The benchmark gains (85.6% CyberGym vs 81.8% GPT-5.5 / 83.8% Claude Mythos 5; 39.5% ExploitGym vs 25.95%; 69.8% SEC-bench Pro vs 63.1%) are now corroborated across many outlets (Axios, SiliconANGLE, Neowin, cybersecuritynews, MLQ) — but EVERY one is non-allowlisted. openai.com still 403s to automated fetch on every relevant page (incl. /index/daybreak-securing-the-world), so no openable allowlisted primary carries the numbers; the OpenAI RSS confirms the items exist but has no figures. Allowlisted TechCrunch covers only the Patch the Planet / Codex Security half — not GPT-5.5-Cyber or any benchmark. As of 23 Jun 20:00 the launch is day-old (no longer net-new). Thematically mirrors T1: the "read a codebase, flag flaws" capability that got Fable 5 pulled is now a shipped OpenAI product.
- **Watch for:** an openable allowlisted primary (openai.com page or model card) confirming GPT-5.5-Cyber + its benchmark numbers; OR a Tier-2 allowlisted outlet (Reuters/AP/Verge/Ars/TechCrunch) carrying the model-release framing that I can open. Given the item is now stale, only a fresh, dated development (independent eval, a defensive deployment result, or a new primary) would still justify a beat. Either openable-primary path clears the gate → ADVANCE.
- **Receipts:**
  - https://openai.com/news/rss.xml (RSS confirms the 22 Jun Daybreak / GPT-5.5-Cyber / Codex Security items; the live post `openai.com/index/daybreak-securing-the-world` still 403s to fetch as of 23 Jun 20:00; benchmarks blog-only)
